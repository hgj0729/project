...
function encodedKey(key){ return /%[0-9A-Fa-f]{2}/.test(key) ? key : encodeURIComponent(key); }
function buildUrl(endpoint,params={}){
  const key=apiKey();
  if(!key){const e=new Error("API 인증키가 설정되지 않았습니다.");e.code="API_KEY_MISSING";throw e;}
  const q=new URLSearchParams({MobileOS:"ETC",MobileApp:"FestOn",_type:"json",...params});
  return `${BASE_URL}/${endpoint}?serviceKey=${encodedKey(key)}&${q.toString()}`;
}
function diagnose(status,text=""){
  const t=text.toUpperCase();
  if(status===403||t.includes("SERVICE_ACCESS_DENIED_ERROR")) return {code:"API_ACCESS_DENIED",message:"403 접근 거부: 해당 관광정보 API의 활용신청/승인 상태와 인증키를 확인하세요."};
  if(t.includes("SERVICE_KEY_IS_NOT_REGISTERED_ERROR")) return {code:"API_KEY_NOT_REGISTERED",message:"등록되지 않은 인증키입니다. 활용신청한 서비스의 인증키를 확인하세요."};
  if(t.includes("SERVICE_KEY_IS_NULL")) return {code:"API_KEY_MISSING",message:"API 인증키가 전달되지 않았습니다."};
  if(t.includes("DEADLINE_HAS_EXPIRED_ERROR")) return {code:"API_KEY_EXPIRED",message:"API 인증키의 이용 기간이 만료되었습니다."};
  if(t.includes("LIMITED_NUMBER")||status===429) return {code:"API_LIMIT_EXCEEDED",message:"API 호출 한도를 초과했습니다."};
  if(status>=500) return {code:"API_SERVER_ERROR",message:"공공데이터 서버에 일시적인 오류가 발생했습니다."};
  return {code:`HTTP_${status}`,message:`API 요청 실패 (HTTP ${status})`};
}

async function request(endpoint, params={}){
  const url=buildUrl(endpoint,params);
  console.log("[TourAPI 요청]",url.replace(/serviceKey=[^&]+/,"serviceKey=***API_KEY***"));
  let res;
  try{
    res=await fetch(url,{method:"GET",headers:{Accept:"application/json, text/plain, */*"}});
  }catch(networkError){
    console.error("[TourAPI 네트워크 오류]",networkError);
    const e=new Error("네트워크 연결 또는 브라우저 보안정책 때문에 API에 연결하지 못했습니다.");e.code="NETWORK_ERROR";throw e;
  }
  const text=await res.text();
  console.log("[TourAPI HTTP 상태]",res.status,res.statusText);
  if(!res.ok){
    console.error("[TourAPI 오류 응답 원문]",text||"(응답 본문 없음)");
    const d=diagnose(res.status,text);
    console.error("[TourAPI 오류 진단]",d);
    const e=new Error(d.message);e.code=d.code;e.status=res.status;e.responseBody=text;throw e;
  }
  let data;
  try{data=JSON.parse(text)}catch(parseError){console.error("[TourAPI JSON 변환 실패]",text);const e=new Error("API 응답이 JSON 형식이 아닙니다.");e.code="INVALID_JSON";throw e;}
  console.log("[TourAPI 전체 응답]",data);
  const rc=data?.response?.header?.resultCode, rm=data?.response?.header?.resultMsg;
  if(rc&&rc!=="0000"){console.error("[TourAPI 응답 오류]",{resultCode:rc,resultMsg:rm});const e=new Error(rm||"API 응답 오류");e.code=rc;throw e;}
  return data;
}
...
async function fetchFestivals(){
  ...
  console.log("[사용할 축제 배열]",list);
  return list;
}

async function fetchDetail(id){
  const results=await Promise.allSettled([
    request("detailCommon2",{contentId:id,defaultYN:"Y",firstImageYN:"Y",addrinfoYN:"Y",overviewYN:"Y",mapinfoYN:"Y"}),
    request("detailIntro2",{contentId:id,contentTypeId:"15"})
  ]);
  if(results[0].status==="rejected"&&results[1].status==="rejected") throw results[0].reason;
  const common=results[0].status==="fulfilled"?(results[0].value?.response?.body?.items?.item?.[0]||{}):{};
  const intro=results[1].status==="fulfilled"?(results[1].value?.response?.body?.items?.item?.[0]||{}):{};
  const detail={...common,...intro};
...
useEffect(()=>{(async()=>{try{const list=await fetchFestivals();if(!list.length)throw Object.assign(new Error("API에서 조회된 축제가 없습니다."),{code:"EMPTY_API_DATA"});setFestivals(list);setDataMode("api");console.info("[FestOn] 공공데이터 API 모드로 실행 중입니다.")}catch(err){console.error("[축제 데이터 오류]",err);setFestivals(mockFestivals);setDataMode("demo");const reason=err.code==="API_KEY_MISSING"?"API 키가 없어":err.message||"API 오류가 발생해";setError(`${reason} 예제 데이터로 자동 전환했습니다.`);console.warn("[FestOn fallback] 샘플 데이터로 자동 전환",{code:err.code,message:err.message})}finally{setLoading(false)}})()},[]);
...
async function openDetail(f){setSelected(f);setScreen("detail");window.scrollTo(0,0);if(dataMode==="api"){try{setSelected({...f,...await fetchDetail(f.contentid)})}catch(e){console.error("[상세 데이터 오류]",e);setError(`상세 API 오류: ${e.message} 목록에서 받은 정보로 표시합니다.`)}}}
